...
use crate::lintrinsics::{LegionellaCustomIntrinsicMirGen, attrs::ConditionItem,
                         attrs::legionella_cfg_attrs, };
...
/// Helper trait for legionella specific custom mirgen impls.
pub trait PlatformIntrinsicInsert {
  fn insert<T>(&mut self, intrinsic: T)
    where T: LegionellaCustomIntrinsicMirGen + fmt::Display
  {
...
pub trait PlatformCodegen: Sized + Clone + Debug + Send + Sync + 'static {
  ...
  /// filtering `legionella_cfg_attr` based on present conditions.
    ...
    -> Lrc<[ast::Attribute]>
  {
    ...
    legionella_cfg_attrs(tcx, &attrs, &*conditions)
  }
