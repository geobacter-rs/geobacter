...
impl<T> WorkDims<T>
  // must be `Copy` due to a match bug.
  where T: Copy,
{
  fn expand_to_three(self, v: T) -> [T; 3] {
    match self {
      WorkDims::One([i]) => [i, v.clone(), v.clone()],
      WorkDims::Two([i, j]) => [i, j, v.clone()],
      WorkDims::Three([i, j, k]) => [i, j, k],
    }
  }
}
